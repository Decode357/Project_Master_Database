...
<div id="CreateShapeCollectionModal" x-show="CreateShapeCollectionModal" x-transition.opacity
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-2xl p-6 overflow-y-auto max-h-[90vh]">
        <h2 class="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">{{ __("content.create_collection") }}</h2>
        ...
        <form @submit.prevent="submitShapeCollectionForm" class="space-y-4" x-data="{
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __("content.collection_code") }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __("content.collection_name") }}</label>
                    <input name="collection_name" type="text" placeholder="{{ __("content.enter") }}{{ __("content.collection_name") }}"
