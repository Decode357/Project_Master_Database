    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 overflow-y-auto max-h-[90vh]">
        ...
        <form @submit.prevent="submitEditForm" class="space-y-4" x-data="{
            ...
            <div>
                ...
                <input type="text" name="product_sku" x-model="productToEdit.product_sku"
                ...
                <p x-show="errors.product_sku"
                    x-text="errors.product_sku ? (Array.isArray(errors.product_sku) ? errors.product_sku[0] : errors.product_sku) : ''"
            ...
            <div>
                ...
                <input type="text" name="product_name" x-model="productToEdit.product_name"
                    :class="errors.product_name ? 'border-red-500' : 'border-gray-300'"
                    class="mt-1 w-full border rounded-md px-3 py-2" required />
                <p x-show="errors.product_name"
                    x-text="errors.product_name ? (Array.isArray(errors.product_name) ? errors.product_name[0] : errors.product_name) : ''"
                    class="text-red-500 text-xs mt-1"></p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="shape_id" x-model="productToEdit.shape_id"
                        ...
                        @foreach ($shapes as $shape)
                            <option value="{{ $shape->id }}">{{ $shape->item_code }} :
                ...
                <div>
                    ...
                    <select name="glaze_id" x-model="productToEdit.glaze_id"
                        :class="errors.glaze_id ? 'border-red-500' : 'border-gray-300'"
                        class="select2 w-full mt-1 border rounded-md px-3 py-2">
                        <option value="">-</option>
                        @foreach ($glazes as $glaze)
                            <option value="{{ $glaze->id }}">{{ $glaze->glaze_code }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="pattern_id" x-model="productToEdit.pattern_id"
                        :class="errors.pattern_id ? 'border-red-500' : 'border-gray-300'"
                        class="select2 w-full mt-1 border rounded-md px-3 py-2">
                        <option value="">-</option>
                        @foreach ($patterns as $pattern)
                            <option value="{{ $pattern->id }}">{{ $pattern->pattern_code }} :
                ...
                <div>
                    ...
                    <select name="backstamp_id" x-model="productToEdit.backstamp_id"
                        :class="errors.backstamp_id ? 'border-red-500' : 'border-gray-300'"
                        class="select2 w-full mt-1 border rounded-md px-3 py-2">
                        <option value="">-</option>
                        @foreach ($backstamps as $backstamp)
                            <option value="{{ $backstamp->id }}">{{ $backstamp->backstamp_code }} :
                                <p>{{ $backstamp->name }}</p>
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex justify-end gap-2 mt-4">
                <button type="button" @click="EditProductModal = false; errors = {}"
