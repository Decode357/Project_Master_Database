...
<div id="EditUserModal" 
    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6" 
        ...
        <div x-show="Object.keys(errors).length > 0" class="p-4 bg-red-100 border border-red-400 rounded-md mb-4">
        ...
        <form @submit.prevent="submitEditForm" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" x-model="userToEdit.name"
                    ...
                    required />
                <p x-show="errors.name" x-text="errors.name ? (Array.isArray(errors.name) ? errors.name[0] : errors.name) : ''" class="text-red-500 text-xs mt-1"></p>
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" x-model="userToEdit.email"
                    ...
                    class="mt-1 w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    required />
                <p x-show="errors.email" x-text="errors.email ? (Array.isArray(errors.email) ? errors.email[0] : errors.email) : ''" class="text-red-500 text-xs mt-1"></p>
            </div>

            <!-- Password (Optional) -->
            <div x-data="{ show: false }" class="relative">
                <label class="block text-sm font-medium text-gray-700">Password <span class="text-gray-400 text-xs">(leave blank to keep current password)</span></label>
                <input :type="show ? 'text' : 'password'" name="password" x-model="userToEdit.password"
                    :class="errors.password ? 'border-red-500' : 'border-gray-300'"
                    class="mt-1 w-full border rounded-md px-3 py-2 pr-10 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <p x-show="errors.password" x-text="errors.password ? (Array.isArray(errors.password) ? errors.password[0] : errors.password) : ''" class="text-red-500 text-xs mt-1"></p>
            </div>
            ...
            <div class="flex flex-row gap-4">
                <div class="flex-1">
                    ...
                    <select name="department_id" x-model="userToEdit.department_id" 
                            ...
                            class="select2 w-full border rounded px-2 py-1">
                        <option value="">-</option>
                        @foreach ($departments as $dep)
                            <option value="{{ $dep->id }}">{{ $dep->name }}</option>
                        @endforeach
                    </select>
                    <p x-show="errors.department_id" x-text="errors.department_id ? (Array.isArray(errors.department_id) ? errors.department_id[0] : errors.department_id) : ''" class="text-red-500 text-xs mt-1"></p>
                </div>
            ...
            <div>
                    ...
                    <template x-for="r in ['user','admin','superadmin']" :key="r">
                        <label
                            :class="{
                                ...
                                'text-gray-500 cursor-not-allowed opacity-50': r === 'superadmin' && currentUserRole === 'admin'
                            }"
                ...
                <p x-show="errors.role" x-text="errors.role ? (Array.isArray(errors.role) ? errors.role[0] : errors.role) : ''" class="text-red-500 text-xs mt-1"></p>
            </div>
            ...
            <div>
                ...
                <p x-show="errors.permissions" x-text="errors.permissions ? (Array.isArray(errors.permissions) ? errors.permissions[0] : errors.permissions) : ''" class="text-red-500 text-xs mt-1"></p>
            </div>
