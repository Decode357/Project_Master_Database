<!-- Glaze Import Modal -->
<div id="glazeImportModal" class="hidden fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 w-11/12 max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800">
        ...
        <div class="flex justify-between items-center pb-3 border-b dark:border-gray-700">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                <i class="fas fa-file-import mr-2"></i>{{ __('content.glaze_data_import') }}
            </h3>
            <button onclick="closeGlazeModal()" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
        ...
        <div class="mt-4">
            ...
            <div class="mb-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <form action="{{ route('glazes.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        <div class="flex space-y-2 flex-row items-center gap-4">
                            <label class="inline-block px-4 py-2 bg-blue-600 text-white rounded-md cursor-pointer hoverScale hover:bg-blue-700 transition dark:bg-blue-500 dark:hover:bg-blue-600">
                                ...
                                <input 
                                    ...
                                    required 
                                    class="hidden" 
                                    onchange="document.getElementById('glazeFileName').textContent = this.files.length ? this.files[0].name : 'No file selected';"
                                />
                            </label>
                            <p id="glazeFileName" class="text-sm text-gray-600 dark:text-gray-300">{{ __('content.no_select_file') }}</p>
                        </div>
                    ...
                    <div class="flex justify-end gap-3">
                        <button type="button" onclick="closeGlazeModal()"
                        ...
                        <button type="button"
                            onclick="if (this.form.reportValidity()) { 
                                document.getElementById('glazeAlertMessages').classList.add('hidden');
                                this.disabled = true; 
            ...
            <div class="p-4 bg-blue-50 dark:bg-blue-900 rounded-lg">
                ...
                <ul class="text-xs text-blue-700 dark:text-blue-300 space-y-1 list-disc list-inside">
                    ...
                    <li>{{ __('content.instruct_2_glaze_data') }}</li>
...
<script>
function openGlazeModal() {
    document.getElementById('glazeImportModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}

function closeGlazeModal() {
    document.getElementById('glazeImportModal').classList.add('hidden');
    document.body.style.overflow = 'auto';
}

// Close modal when clicking outside
document.getElementById('glazeImportModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
        closeGlazeModal();
    }
});
...
@if((session('error') || session('import_errors') || session('success')) && session('glaze_import'))
