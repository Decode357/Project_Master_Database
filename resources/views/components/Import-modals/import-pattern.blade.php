...
<div id="patternImportModal" class="hidden fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 w-11/12 max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800">
        ...
        <div class="flex justify-between items-center pb-3 border-b dark:border-gray-700">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                <i class="fas fa-file-import mr-2"></i>{{ __('content.pattern_data_import') }}
            </h3>
            <button onclick="closePatternModal()" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
        ...
        <div class="mt-4">
            ...
            <div class="mb-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('patterns.template') }}"
            ...
            <div class="mb-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg opacity-50">
                ...
                <form action="{{ route('patterns.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        <div class="flex space-y-2 flex-row items-center gap-4">
                            <label class="inline-block px-4 py-2 bg-blue-600 text-white rounded-md cursor-pointer hoverScale hover:bg-blue-700 transition dark:bg-blue-500 dark:hover:bg-blue-600">
                                ...
                                <input 
                                    ...
                                    required 
                                    class="hidden" 
                                    onchange="document.getElementById('patternFileName').textContent = this.files.length ? this.files[0].name : 'No file selected';"
                                />
                            </label>
                            <p id="patternFileName" class="text-sm text-gray-600 dark:text-gray-300">{{ __('content.no_select_file') }}</p>
                        </div>
                    ...
                    <div class="flex justify-end gap-3">
                        <button type="button" onclick="closePatternModal()"
                        ...
                        <button type="button"
                            onclick="if (this.form.reportValidity()) { 
                                document.getElementById('patternAlertMessages').classList.add('hidden');
                                this.disabled = true; 
...
function openPatternModal() {
    document.getElementById('patternImportModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}

function closePatternModal() {
    document.getElementById('patternImportModal').classList.add('hidden');
    document.body.style.overflow = 'auto';
}

// Close modal when clicking outside
document.getElementById('patternImportModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
        closePatternModal();
    }
});
...
@if((session('error') || session('import_errors') || session('success')) && session('pattern_import'))
