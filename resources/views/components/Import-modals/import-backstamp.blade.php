<!-- Backstamp Import Modal -->
<div id="backstampImportModal" class="hidden fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 w-11/12 max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800">
        ...
        <div class="mt-4">
            <!-- Alert Messages -->
            <div id="backstampAlertMessages">
                @if(session('success')&& session('backstamp_import'))
                ...
                @if(session('import_errors') && is_array(session('import_errors'))&& session('backstamp_import'))
            ...
            <div class="mb-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('backstamps.template') }}"
                        ...
                        <div class="flex space-y-2 flex-row items-center gap-4">
                            <label class="inline-block px-4 py-2 bg-blue-600 text-white rounded-md cursor-pointer hoverScale hover:bg-blue-700 transition dark:bg-blue-500 dark:hover:bg-blue-600">
                                ...
                                <input 
                                    ...
                                    required 
                                    class="hidden" 
                                    onchange="document.getElementById('backstampFileName').textContent = this.files.length ? this.files[0].name : 'No file selected';"
                                />
                            </label>
            ...
            <div class="p-4 bg-blue-50 dark:bg-blue-900 rounded-lg">
                ...
                <ul class="text-xs text-blue-700 dark:text-blue-300 space-y-1 list-disc list-inside">
                    ...
                    <li>{{ __('content.instruct_2_backstamp_data') }}</li>
...
<script>
function openBackstampModal() {
    document.getElementById('backstampImportModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}

function closeBackstampModal() {
    document.getElementById('backstampImportModal').classList.add('hidden');
    document.body.style.overflow = 'auto';
}

// Close modal when clicking outside
document.getElementById('backstampImportModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
        closeBackstampModal();
    }
});
...
@if((session('error') || session('import_errors') || session('success')) && session('backstamp_import'))
