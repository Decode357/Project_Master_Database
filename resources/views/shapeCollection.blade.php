...
@section('title', __('sidebar.shape_collections'))
@section('header', __('sidebar.shape_collections'))
@section('content')
    <main class="flex-1 bg-gray-50 dark:bg-gray-900" x-data="shapeCollectionPage()" x-init="initSelect2()">

        <!-- Filters -->
        <div class="bg-white p-6 rounded-lg shadow-md mb-3 
            dark:bg-gray-800 dark:shadow-gray-900/50">
            <form method="GET" action="{{ route('shape.collection.index') }}" class="flex flex-wrap items-end gap-4">
                <!-- Search Input -->
                <div class="flex-1 min-w-64">
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">search</span>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('content.search_by') }}{{ __('content.shape_collection_code') }},{{__('content.shape_collection_name')}}{{ __('content.etc') }}"
            ...
            <div class="overflow-x-auto rounded-xl">
                <table class="w-full text-sm text-left text-gray-600
                    ...
                    <thead class="text-xs text-gray-500 uppercase bg-gray-50 border-b dark:border-gray-700
                        ...
                        <tr>
                            <th class="px-6 py-3">{{ __('content.shape_collection_code') }}</th>
                            <th class="px-6 py-3">{{ __('content.shape_collection_name') }}</th>
                        ...
                        @forelse ($shapeCollections as $collection)
                            <tr class="bg-white border-b hover:bg-gray-50 
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    @endif
                                    @if ($hasDelete)
                                        <button @click="DeleteShapeCollectionModal = true; shapeCollectionIdToDelete = {{ $collection->id }}; itemCodeToDelete = '{{ $collection->collection_code }}'"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-sm text-gray-500 text-center
                                    dark:text-gray-400">
                                    @if(request('search'))
                                        No shape collections found for "{{ request('search') }}".
                                    @else
                                        No shape collections found.
                                    @endif
                                </td>
                            </tr>
                        @endforelse
                ...
                <div class="mt-4 flex justify-end pb-2">
                    {{ $shapeCollections->links('vendor.pagination.tailwind-custom') }}
                </div>
            </div>
        </div>
        {{-- include modal --}}
        @include('components.Edit-modals.edit-shapeCollection')
        @include('components.Create-modals.create-shapeCollection')
        <x-modals.delete-modal 
            show="DeleteShapeCollectionModal"
            itemName="itemCodeToDelete"
            deleteFunction="deleteShapeCollection"
        />    
    </main>
@endsection
